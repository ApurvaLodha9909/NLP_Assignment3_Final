...
"""
'''train_weights_input2 = tf.Variable(tf.truncated_normal(shape=[constants.hidden_size, constants.hidden_size], stddev=0.1))
            train_bias_input2 = tf.Variable(tf.zeros([constants.hidden_size, 1]))
...
class DependencyParserModel(object):
    ...
    def train(self, sess, num_steps):
        ...
        for step in range(num_steps):
            start = (step * constants.batch_size) % len(trainFeats)
            end = ((step + 1) * constants.batch_size) % len(trainFeats)
            if end < start:
                start -= end
                end = len(trainFeats)
            ...
            if step % constants.display_step == 0:
                if step > 0:
                    average_loss /= constants.display_step
                print "Average loss at step ", step, ": ", average_loss
                average_loss = 0
            if step % constants.validation_step == 0 and step != 0:
    ...
    def evaluate(self, sess, testSents):
        ...
        Util.writeConll('test_predictions.conll', testSents, predTrees)
...
def genDictionaries(sents, trees):
    ...
    index = 0
    wordCount = [constants.UNKNOWN, constants.NULL, constants.ROOT]
    wordCount.extend(collections.Counter(word))
    for word in wordCount:
        wordDict[word] = index
        index += 1

    posCount = [constants.UNKNOWN, constants.NULL, constants.ROOT]
    posCount.extend(collections.Counter(pos))
    for pos in posCount:
        posDict[pos] = index
        index += 1

    labelCount = [constants.NULL, rootLabel]
...
def getWordID(s):
    if s in wordDict:
        ...
        return wordDict[constants.UNKNOWN]
...
def getPosID(s):
    if s in posDict:
        ...
        return posDict[constants.UNKNOWN]
...
def getLabelID(s):
    if s in labelDict:
        ...
        return labelDict[constants.UNKNOWN]
...
def load_embeddings(filename, wordDict, posDict, labelDict):
    ...
    embedding_array = np.zeros((len(wordDict) + len(posDict) + len(labelDict), constants.embedding_size))
    ...
    for i in range(len(embedding_array)):
        ...
        if index >= 0:
            ...
            embedding_array[i] = np.random.rand(constants.embedding_size) * 0.02 - 0.01
